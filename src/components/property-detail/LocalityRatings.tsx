...
export default function LocalityRatings({ property }: Props) {
  ...
  const getScore = (val?: number) => (val && val > 0 ? val : 4);
  const getPct = (val?: number) => (val && val > 0 ? `${(val / 5) * 100}%` : "80%");
  ...
  const localityText =
    n.notes ||
    `A well-connected and vibrant neighbourhood in ${property.locality || "the area"}, ${property.city || "this city"} with excellent infrastructure, safe environment, and strong property appreciation potential.`;
